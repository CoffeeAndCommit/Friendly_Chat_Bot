...
SYSTEM_INSTRUCTIONS = (
    "You are an assistant that has access to a short summary of the user's past "
    "conversations. Use it to personalize replies when relevant. Be concise and friendly."
)
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # user_id comes from routing: path("ws/chat/<str:user_id>/", ChatConsumer.as_asgi())
        path_user_id = self.scope.get("url_route", {}).get("kwargs", {}).get("user_id")

        # Decide external_id used by memory.tables
        if self.user and getattr(self.user, "is_authenticated", False):
            self.external_id = f"user:{self.user.pk}"
        elif path_user_id:
            # this matches your frontend: ws://localhost:8000/ws/chat/123/
    ...
    def _get_summary_text(self):
        """
        Synchronous helper to fetch ConversationSummary.summary_text for this external_id.
        Import models here to avoid AppRegistryNotReady at import-time.
        """
        try:
